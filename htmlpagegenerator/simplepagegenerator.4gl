...
DEFINE root, head, body, script, p, div, button, text_node xml.DomNode
    ...
    if(typeof(Storage) !== "undefined") {\
        ...
        document.getElementById("result_session").innerHTML = "You have clicked the button " + sessionStorage.clickcount + " time(s) in this session.";\
    } else {\
    ...
    if(typeof(Storage) !== "undefined") {\
        ...
        document.getElementById("result_local").innerHTML = "You have clicked the button " + localStorage.clickcount + " time(s) in this session.";\
    } else {\
    ...
    LET p = body.appendChildElement("p")
    LET text_node = doc.createTextNode("Close the browser tab (or window), and try again, and the counter is reset.")
    CALL p.appendChild(text_node)
